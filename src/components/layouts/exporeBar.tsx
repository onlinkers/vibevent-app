...
const ExploreBar: React.FunctionComponent<Props> = () => {
  ...
  return innerWidth >= 800 ? (
    <div className="desktop-explore">
      <Link to="/">
        <h2 className="explore-title" color="blue">
          ONLINKER
        </h2>
      </Link>
      <div className="explore-links">
        <Link to="/event/dashboard">
          <div className="explore-link">
            <p
              className={`explore-link-text ${
                pathname.includes(EXPLORE_STATES.DASHBOARD) && "active"
              }`}
            >
              Dashboard
            </p>
            {pathname.includes(EXPLORE_STATES.DASHBOARD) && (
              <div className="explore-link-line" />
            )}
          </div>
        </Link>
        <Link to="/discover">
          <div className="explore-link">
            <p
              className={`explore-link-text ${
                pathname.includes(EXPLORE_STATES.DISCOVER) && "active"
              }`}
            >
              Discover
            </p>
            {pathname.includes(EXPLORE_STATES.DISCOVER) && (
              <div className="explore-link-line" />
            )}
          </div>
        </Link>
        <Link to="#">
          <div className="explore-link">
            <p
              className={`explore-link-text ${
                pathname.includes(EXPLORE_STATES.MOMENTS) && "active"
              }`}
            >
              Moments
            </p>
            {pathname.includes(EXPLORE_STATES.MOMENTS) && (
              <div className="explore-link-line" />
            )}
          </div>
        </Link>
        <Link to="/event/create">
          <div className="explore-link">
            <p
              className={`explore-link-text ${
                pathname.includes(EXPLORE_STATES.CREATE) && "active"
              }`}
            >
              Create
            </p>
            {pathname.includes(EXPLORE_STATES.CREATE) && (
              <div className="explore-link-line" />
            )}
          </div>
        </Link>
      ...
      <Link to="/profile">
        <div className="explore-profile">
          <UserOutlined
            style={
              pathname.includes(EXPLORE_STATES.PROFILE)
                ? { color: "#2351e0" }
                : { color: "#000" }
            }
          />
    ...
    <div className="mobile-explore">
      ...
      <div className="explore-mobile-button">
        <ExploreButton />
      </div>
    </div>
  );
